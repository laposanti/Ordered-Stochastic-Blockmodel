...
adaptive_MCMC_orderstats <- function(Y_ij, N_ij,init , estimation_control, 
  ...
  reprex <- local({
    ...
    y <- foreach(chain = 1:n_chains,.options.future = list(globals = structure(TRUE, add='init'), seed=TRUE)) %dofuture% { 
      ...
      U_vec_container = matrix(0, nrow = K_max, ncol = N_iter)
      ...
      A_current = llike_integrated_out(lambdabar, ybar,mbar,K,U_vec, sigma_squared)
      ...
      for(j in 1:N_iter){
        ...
        if (estimation_control$P == 1) {
          ...
          P_update = P_update_f(lambdabar = lambdabar,ybar = ybar,mbar = mbar,
        ...
        if (estimation_control$a == 1) {
          ...
          #alpha UPDATE----------------------------------------------------------------
          a_update = a_update_f_withP(lambdabar = lambdabar, ybar = ybar,mbar = mbar,  
        ...
        if (estimation_control$sigma_squared == 1) {
          ...
          sigma_squared_update <- sigma_squared_update_f_withP(lambdabar = lambdabar, P = P_current,
                                                               ybar = ybar,mbar=mbar, 
                                                               a = a_current, 
        ...
        if (estimation_control$U== 1) {
          ...
          U_update=  U_update_f_withP(lambdabar = lambdabar, ybar = ybar,mbar = mbar,  P = P_current,
          ...
          a_container[j]<- a_current
          sigma_squared_container[j] = sigma_squared_current
      ...
      est_containers = list(z = z_container, alpha = a_container,P = P_container,

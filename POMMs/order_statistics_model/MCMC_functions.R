...
llike_integrated_out = function(lambdabar, ybar,mbar,K,U_vec, sigma_squared){
  ...
  for(p in 1:K){
    for(q in p:K){
      if(q-p != 0){
        ...
        llik[p,p]<-  log(lambdabar[p,p]) +lbeta(1 + ybar[p,p],1 
                                                + ybar[p,p]) - lbeta(1, 1)  
      }
    }
  }
...
lprop_posterior_P_out <- function(lambdabar, ybar,mbar, a, 
                                  alpha_vec, n_k,sigma_squared, U_vec,K){
  
  sum1 <- llike_integrated_out(lambdabar, ybar,mbar,K,U_vec, sigma_squared)
  #prior on z
...
#-------------------------- MCMC steps -----------------------------------------
a_update_f_P_out = function(lambdabar, ybar,mbar, a, alpha_vec, n_k,
...
U_update_f_P_out = function(lambdabar, ybar,mbar, a, alpha_vec, n_k,
...
sigma_squared_update_f_P_out = function(lambdabar,ybar,mbar, a, alpha_vec, n_k,
  ...
  prop_posterior_current<- lprop_posterior(lambdabar, ybar,mbar, a, alpha_vec, 
...
llik_over_blocks_f_binomial = function(lambdabar, ybar, mbar, P){
  #llik<- sum(bin_coef+ ybar*log(P) + (mbar)*log(1-P))
  llik = matrix(0,K,K)
  for(p in 1:K){
    for(q in p:K){
      if(q-p==0){
        llik[p,q] = lambdabar[p,q]+ ybar[p,q]*log(P[p,q]) + mbar[p,q]*log(1-P[p,q])
      }else{
        llik[p,q] = lambdabar[q,p]+ lambdabar[p,q]+(ybar[p,q]+mbar[q,p])*log(P[p,q]) + (mbar[p,q]+ybar[q,p])*log(1-P[p,q]) 
      }
    }
  }
  return(sum(llik))
...
lprop_posterior_withP <- function(lambdabar, ybar,mbar,P, a, 
                                  alpha_vec, n_k,sigma_squared, U_vec,K){
  
  sum1 <- llik_over_blocks_f_binomial(lambdabar, ybar, mbar, P)
  #prior on P
...
P_update_f = function(lambdabar,ybar,mbar,P, a, alpha_vec, n_k,
  ...
  for(p in 1:K){
    for(q in p:K){
      ...
      P_prime<-P
      P_prime[p,q]<- rtruncnorm(1, mean = P[p,q],sd = tau_P[p,q], a =   1-a, b  = a)
      if(p!=q){
      P_prime[q,p] <- 1 - P_prime[p,q]
      }
      #computing the proportional posterior in a'
      prop_posterior_prime <-  lprop_posterior_withP(lambdabar, ybar,mbar,P_prime, a, 
                                                     alpha_vec, n_k,sigma_squared, U_vec,K)
      
      #evaluating the proportional posterior in a^(t)
      prop_posterior_current<- lprop_posterior_withP(lambdabar, ybar,mbar,P, a, 
                                                     alpha_vec, n_k,sigma_squared, U_vec,K)

      #evaluating the proposal density g(a') 
      log_proposal_prime <- log(dtruncnorm(P_prime[p,q],mean = P[p,q],sd = tau_P[p,q], a =   1-a, b  = a))
      
      #evaluating the proposal density g(sigma^2)^(t)) 
      log_proposal_current <- log(dtruncnorm(P[p,q],mean = P_prime[p,q],sd = tau_P[p,q], a =   1-a, b  = a))
...
a_update_f_withP = function(lambdabar, ybar,mbar, P,a, alpha_vec, n_k,
                            sigma_squared, U_vec,K, tau_a,
                            acc.count_a){
  ...
  #computing the proportional posterior in a'
  prop_posterior_prime <-  lprop_posterior_withP(lambdabar, ybar,mbar,P, a_prime, 
                                                 alpha_vec, n_k,sigma_squared, U_vec,K)
...
U_update_f_withP = function(lambdabar, ybar,mbar,P, a, alpha_vec, n_k,
                            sigma_squared, U_vec,K, tau_U_vec,
                            acc.count_U){
  # 
  # for(k in 2:(K+1)){
  ...
  #computing the proportional posterior in a'
  prop_posterior_prime <- lprop_posterior_withP(lambdabar,ybar,mbar,P,a_current,alpha_vec,n_k,sigma_squared,U_vec = U_vec_prime,K)
  
  #evaluating the proportional posterior in a^(t)
  prop_posterior_current <- lprop_posterior_withP(lambdabar,ybar,mbar,
...
sigma_squared_update_f_withP = function(lambdabar,ybar,mbar,P, a, alpha_vec, n_k,
                                        sigma_squared, U_vec,K, tau_sigma_squared,
                                        acc.count_sigma_squared){
  ...
  #computing the proportional posterior in (sigma^2)'
  prop_posterior_prime <- lprop_posterior_withP(lambdabar, ybar,mbar,P, a, 
...
z_update_f_withP = function(N_ij, Y_ij, z, P, a, alpha_vec, n_k,
  ...
  for(ii in scanning_order){
    ...
    B_scanning<- ddirichlet_multinomial(N,K,n_k = n_scanning,my_alpha = alpha_vec)

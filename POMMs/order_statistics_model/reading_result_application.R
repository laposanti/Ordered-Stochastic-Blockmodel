...
data_wd<- "/Users/lapo_santi/Desktop/Nial/MCMC_results/simulation_study_orderstats/raw/"

processed_wd <- "/Users/lapo_santi/Desktop/Nial/MCMC_results/simulation_study_orderstats/SST_true/"
...
for(est_model in c("SST","WST", "Simple")){
  ...
  for(file in 1:length(filenames)){
    ...
    z_tot_table<- z_summary_table(chains  = uploaded_results, true_value = T, 
    ...
    if(est_model !=  'Simple'){
      ...
      a_s_table<- a_summary_table(chains = uploaded_results, true_value = T, 
                                  diag0.5 = TRUE, K = K, burnin = N_iter*0.25)
    ...
    P_d_table<- P_diagnostic_table(chains = uploaded_results, true_value = T, 
    ...
    if(est_model != 'Simple'){
      ...
      U_vec_d_table = U_vec_d_table %>% mutate(model= est_model)%>% mutate(n_clust = K)

      if(est_model=='SST'&file==1){
        U_vec_d_container = U_vec_d_table
      }else{
        U_vec_d_container =  rbind(U_vec_d_container,U_vec_d_table)
      }
    ...
    #---------------------------------------------------------------------------
    P_est_title <- paste0(processed_wd,'/P_est_matrix',filenames[file], '.csv')
    P_est <- round(P_s_table$P_hat,3) %>% data.frame() 
    P_est %>% write.csv(file = P_est_title)
    
    P_true_title <- paste0(processed_wd,'/P_true_matrix',true_model, '.csv')

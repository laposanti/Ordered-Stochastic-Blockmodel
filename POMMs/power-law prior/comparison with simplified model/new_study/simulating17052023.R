...
label_switch = label.switching(method = "DATA-BASED",z = t(z.container[,-c(1:N_iter*0.5)]),K = K_true, data=rowSums(y_ij_matrix+n_ij_matrix))
...
adj.rand.index(z.true, point_est)


similarity_plot(y_ij_matrix,point_est2,point_est2)
...
alpha.container=alpha.container[-c(1:10000)]
...
mean_alpha= mean(alpha.container)
print(mean_alpha)
alpha_df= data.frame(y=alpha.container)

sd_alpha = sqrt(var(alpha.container))
...
for(i in 1:K_true){
  for(j in 1:K_true){
    print((mean(p.container[i,j,]) - synth$P_matrix[i,j])*100)
  }
}

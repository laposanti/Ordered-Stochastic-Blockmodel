...
K_list = c(3,5,8)
my_seed =12
pomm_model_estimation = function(){
while(my_seed<13){
  ...
  p_current.truncations = simulating_POMM_powerlaw(K_true,alpha,beta_max)$truncations + c(0,runif(2,0,0.1),0)
  
  df_fast =df_aux_fast(synth_matches,synth_players,synth_p)
  
  delta.p = c(0,rep((beta_max-0.5)/K_true,K_true))
  x_points = cumsum(delta.p)
  lx_points = log(x_points)[-1]
  ...
  while (j < N_iter + 1) {
    ...
    # sampled_order = sample(c(0:(K_true-1)),3,replace = F)
    ...
    #   p_scanning = semi_symmetric(p_scanning)
    #   #tranforming data
    ...
    #   
    #   max_diag = matrix(0,K_true,1)
    #   for(u in (K_true):1){
    #   my_split = diag_split_matrix(p_scanning)
    #   max_diag[K_true+1-u] = max(my_split[[u]])}
    #   #adjusting alpha with new value
    #   y_log = log(max_diag - 0.5)
    #   fit <- lm(y_log ~ lx_points)
    #   alpha_scanning = coef(fit)[2]
    #   
    #   #modifying alos the (iii+1)th truncation
    #   p_scanning.truncations = simulating_POMM_powerlaw(K_true,alpha_scanning,beta_max)$truncations
    #   
    #   
    #   r = (sum(dbinom(y_ij, n_ij, p_ij_scanning, log=T)) +  l_like_p_ij(p_scanning,p_scanning.truncations)) - 
...
system.time(pomm_model_estimation())
  #diagnostic for z
  #-----
  ...
  for(i in 1:K_true){
    for(j in 1:K_true){
      mse_table[i,j]= (mean(p.container[i,j,]) - synth_p[i,j])*100
    }
  }
...
heat_map_blue(synth_p,"An example of the P matrix")

alpha = 1.5
synth_p = simulating_POMM_powerlaw(3,alpha,0.75)$matrix
K = nrow(synth_p)
# convert matrix to data frame
df <- as.data.frame(t(synth_p))
x = c(1:K)
y= c(K:1)
...
ggplot(df_plot, aes(x=X, y=Y, fill=Z)) +
  ...
  labs(title=paste("An example of the P matrix with alpha = ", alpha), x="Column", y="Row", fill="Prob")+
  geom_text(aes(label = round(Z, 2), col= Z>0.5), size = 20/K,show.legend = F,fontface = "bold")

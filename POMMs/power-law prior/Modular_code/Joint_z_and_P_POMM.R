...
p_current = simulating_POMM_powerlaw1(K,alpha_current,truncations_current,beta_max)$matrix
...
C_current =  l_like_p_ij(p_current,truncations_current) + dlnorm_param(alpha_current)
...
while (j < N_iter + 1) {
  ...
  p_update= P_POMM_update(z_current = z_current,
                          P_matrix = p_current,
                          K = K,n_ij = n_ij,
                          y_ij = y_ij,

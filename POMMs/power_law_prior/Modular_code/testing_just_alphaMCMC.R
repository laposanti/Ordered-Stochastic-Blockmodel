...
while (j < N_iter + 1) {
  ...
  p_update= alpha_POMM_update1(z_current = z_current,
                           p_current = p_current,
                           K = K,n_ij = n_ij,
                           y_ij = y_ij,
...
alpha = 3
test_trunc = improper_prior5(7,.85,3,T)
l_like_p_ij_normal(K,p_current,truncations = test_trunc,T)

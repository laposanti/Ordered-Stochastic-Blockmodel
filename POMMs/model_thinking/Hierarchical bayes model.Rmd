...
& \sigma^2 \sim f(\sigma^2) = \operatorname{TruncExp}(\lambda_{\sigma^2}=1\mid \sigma^2 < (U^{\max}\cdot(1-U^{\max}))) , \quad \text{where } U^{\max}= \max \left(\mathbf{U}_{(1:K)}\right)\\
...
&\cdot \underbrace{K! \cdot \left( \frac{1}{a - 0.5}\right)^K}_{ f\left(\mathbf{U}_{(1:K)} \mid a, K\right)} \cdot \underbrace{\frac{e^{-\sigma^2}}{1-\exp \left(U^{\max }(1-U^{\max }\right)}}_{f\left(\sigma^2\right)} \underbrace{\frac{1}{2}}_{ f(a)} \cdot \underbrace{\frac{1}{K!(e-1)}}_{f(K)}
\end{align}
...
&\log \left(1-\exp \left(U^{\max }(1-U^{\max })\right)\right)+ \log (1 / 2)-\log (K !)-\log (e-1) \\
...
lprop_posterior <- function(lambdabar, ybar,mbar, a, 
  ...
  #prior on sigma^2
  sum4 <- -sigma_squared - log(1-exp(max(U)*(1-max(U))))
  #prior on K
...
\begin{align} (\sigma^{2})^{\prime}&\sim g \left( (\sigma^{2})^{(t)}, (\tau^2_{\sigma^2})^{(t)}\mid 0, U^{\max}\right) \\
&= \operatorname{TruncNorm}\left(\mu= (\sigma^{2})^{(t)}, \sigma^2 = (\tau^2_{\sigma^2})^{(t)} , lb= 0,ub= U^{\max}(1-U_{\max})\right)\end{align}

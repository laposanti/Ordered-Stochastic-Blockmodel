...
data_wd<-  '/Users/lapo_santi/Desktop/Nial/MCMC_results/first_attempt_basket_data/'

#where the data will be saved
tap <- '/Users/lapo_santi/Desktop/Nial/MCMC_results/first_attempt_basket_data/processed/'
plots_dir<- '/Users/lapo_santi/Desktop/Nial/MCMC_results/first_attempt_basket_data/processed/'
...
while(controller==0){
  ...
  true_model <- 'Basket_application_Est_model_'
  ...
  for(results_row in c(F,T)){
    ...
    est_model<- 'POMM'
    filename <- list.files(pattern = paste0('Basket_data_Est_model_', est_model),path = data_wd)
    print(filename)
    ...
    for(i in 1:length(filename)){
      ...
      est_df<- data.frame(player_slug = rownames(uploded_results$chain1$Yij_matrix), est_cl = z_tot_table$memb, rank=c(1:15))

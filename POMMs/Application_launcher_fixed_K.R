...
for(data_description in c("Tennis_data","Citation_data")){
  ...
  for(diag0.5 in (TRUE,FALSE)){
    ...
    K_values <- c(3:15)  # Range of K values to explore
    ...
    for(k_th in K_values){
      ...
      seed = 20
      N_iter <- 70000 + 10000*(k_th-2) #number of iterations
      burnin <- 50000 + 10000*(k_th-2) #number of discarded iterations
      ...
      if('SST' %in% choose_model_to_estimate){
        ...
        my_filename = paste0(saving_directory, '/Data_from',data_description, "_est_model",
                             est_model,"_Kest",paste(unlist(K_est),collapse = "_"),'.rds')
      ...
      if('WST' %in% choose_model_to_estimate){
        ...
        my_filename = paste0('./Results/MCMC_output/Fixed_K/Application/Data_from',data_description, "_est_model",est_model,"_Kest",paste(unlist(K_est),collapse = "_"),'.rds')
      ...
      if('Simple' %in% choose_model_to_estimate){
        ...
        my_filename = paste0(saving_directory,'/Data_from',
                             data_description, "_est_model",est_model,"_Kest",paste(unlist(K_est),collapse = "_"),'.rds')

...
There exists a correspondence between posets and directed cyclic graphs, which is useful to elicit the dependency structure between $P$ elements.

Let $(D, F)$ be a directed acyclic graph, where $D = \{y_1, \ldots, y_n\}$, a finite set. To construct the digraph corresponding to the poset $P$, we define the edge set $F= \{(x_i, x_j) \in D \text{ s.t. } x_i \prec x_j$
\begin{align}

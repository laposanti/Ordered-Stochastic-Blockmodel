...
df_match <- read.csv('/Users/lapo_santi/Desktop/Nial/POMM_pairwise/POMMs/Basket application/Basket data.csv')
df_rank <- read.csv('/Users/lapo_santi/Desktop/Nial/POMM_pairwise/POMMs/Basket application/Ranking.csv')
df_rank <- df_rank %>% select(name, rank)
df_edgelist<- data.frame(winner_slug = ifelse(df_match$pt.A>df_match$pt.B,yes =df_match$teamA_slug,no=df_match$teamB_slug),
                         loser_slug = ifelse(df_match$pt.A<df_match$pt.B,yes =df_match$teamA_slug,no=df_match$teamB_slug),
                         winner_name = ifelse(df_match$pt.A>df_match$pt.B,yes =df_match$teamA,no=df_match$teamB) ,
                         loser_name = ifelse(df_match$pt.A<df_match$pt.B,yes =df_match$teamA,no=df_match$teamB))
df_edgelist<- df_edgelist %>% inner_join(df_rank, by = c('winner_name'='name') ) 
df_edgelist<- df_edgelist %>% rename(winner_rank = rank)
df_edgelist<- df_edgelist %>% inner_join(df_rank, by = c('loser_name'='name') ) 
df_edgelist<- df_edgelist %>% rename(loser_rank = rank)

my_edges = df_edgelist %>% select(winner_name, loser_name)
...
selecting_topx_andbottomy <- function(e, from_x, to_x, from_y, to_y, rank_df){
  ...
  from_x_players <- df_rank %>% arrange(rank) %>% select(name) %>% head(from_x-1)
  to_x_players <- df_rank %>% arrange(rank) %>% select(name) %>% head(to_x)
  from_xto_x_players<- setdiff(unlist(to_x_players), unlist(from_x_players))
  
  from_y_players <- df_rank %>% arrange(rank) %>% select(name) %>% head(from_y-1)
  to_y_players <- df_rank %>% arrange(rank) %>% select(name) %>% head(to_y)
  ...
  for (i in 1:nrow(e)) {
    winner <- as.character(e$winner_slug[i])
    loser <- as.character(e$loser_slug[i])
  ...
  e_df$marginal_x<- rep(0,nrow(e_df))
  for(i in 1:nrow(e_df)){
    e_df$marginal_x[i]<- sum(A_xvsy[,e_df$Player_y[i]],na.rm = T)/sum(N_xvsy[,e_df$Player_y[i]],na.rm = T)
  }
...
for(i in 1:length(range_list)){
  ...
  list_pl <-selecting_topx_andbottomy(e=e,bottom_x,top_x,1,29)
...
selecting_topx_andbottomy(e, 1,15,1,15)
# Heatmap
#-------------------------------------------------------------------------------
...
tile_df1 = tile_df1 %>% rename(loser_rank = median_rank)
...
ggplot(summary_df, aes( y = probabilities, fill = range_pl)) +
  geom_boxplot() +
  facet_wrap(~ factor(range_pl),nrow = 1) +
  labs(title = paste0("Victories/Matches distribution of Players", range_x," vs. Different Ranges"),
       x = "Players",

...
synth = simulating_overlapping_POMM_powerlaw_norm(N = N, alpha = alpha,
                                       beta_max = beta_max,
                                       K=K, M = M,overlap=overlap,
                                       gamma_vec = gamma_vec,
...
initial_overlap<- matrix(0,1,num_cores)
...
for(i in 1:num_cores){
  ...
  #initialising overlap
  overlap_current = runif(1,0.1,0.9)
  initial_overlap[i]<-overlap_current
...
results <- foreach(chain_index = 1:num_cores, .packages = c("progress")) %dopar% {
  ...
  z_current <- initial_z[,chain_index]  
  p_current <- initial_p[,,chain_index]  
  A_current <- initial_A[chain_index]  
  B_current <- initial_B[chain_index]  
  C_current <- initial_C[chain_index]  
  alpha_current <- initial_alpha[chain_index]  
  overlap_current<- initial_overlap[chain_index]

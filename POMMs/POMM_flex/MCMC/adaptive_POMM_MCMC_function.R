...
adaptive_MCMC_POMM <- function(Yij_matrix, Nij_matrix,init , estimation_control, ground_truth,N, N_iter,n_chains, targ_rate, hyper_params, seed){
  
  handlers(global = TRUE)
  registerDoFuture()
  plan(multisession, workers=max(n_chains,(detectCores()-2)))
    ...
    y <- foreach(chain = 1:n_chains,.options.future = list(globals = structure(TRUE, add='init'), seed=TRUE)) %dofuture% { 
      ...
      for(j in 1:N_iter){
        ...
        if(j%%5000==0){
          ...
          p(sprintf("Chain %d: mean acc.rate z %.3f%%, mean acc.rate P %.3f%%,mean acc.rate alpha %.3f%%,mean acc.rate S %.3f%%, single_iter_time '%*.3f' seconds, will_finish_at '%s'",
                    chain,  
                    100 * mean(acc.count_z/j),
                    100 * mean(acc.count_p/j),
                    100 * mean(acc.count_alpha/j),
                    100 * mean(acc.count_S/j),
      ...
      return(list(Yij_matrix=Yij_matrix, Nij_matrix=Nij_matrix,

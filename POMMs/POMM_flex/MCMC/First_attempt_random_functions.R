...
for(i in 1:n_samples){
  trunc = improper_prior5(K,alpha = alpha,diag0.5 = F, beta_max = beta_max )
  p_container[,,i] = simulating_POMM_powerlaw1(K, alpha=alpha, beta_max = beta_max,truncations = trunc)$matrix
}
...
l_like_p_ij2 = function(K, P_matrix, truncations, diag0.5 = T) {
  #K <- ifelse(diag0.5, K - 1, K)
  K<- K-1
  level_sets <- diag_split_matrix(P_matrix)
  ...
  for (i in (1 + lbindex):(K + 1)) {
    ...
    # Calculate the log-likelihood using the truncated distribution
    log_lik <- log_lik + sum(log(dunif(unlist(level_set_i), min =  lb, max = ub)))/length(level_set_i)
  }
...
for(j in 1: n_samples){
  for(i in 1:length(alpha_test)){
    ...
    likelihood_est[j,i]= l_like_p_ij2(K,p_container[,,j],trunc_i,diag0.5 = F) + dlnorm_param(alpha_test[i])
  }
}

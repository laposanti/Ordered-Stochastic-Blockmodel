...
P_POMM_update_given_overlap = function(z_current, p_current, 
                                       A_current,C_current,y_ij,n_ij,labels_available,
                                       upper.tri.non.zero,K,alpha_current,beta_max, overlap_current, diag0.5,acc.value_p){
  
  z_current_mat<- vec2mat(z_current)
  acc.value_p<- matrix(0, K,K)
  
  
  
  
  #proposing a new P
  p_proposal <-  simulating_overlapping_POMM_powerlaw_norm(K, alpha_current,
                                                        truncations_current,
  ...
  for( ii in 1:K_stop){
    for(jj in (ii+j_start):K){
      ...
      p_prime <- p_current
      p_prime[ii,jj] <-p_proposal[ii,jj]
      p_prime[jj,ii] <-p_proposal[jj,ii]
      ...
      if(MH_condition){
        acc.value_p[ii,jj] =acc.value_p[ii,jj] +1
        acc.value_p[jj,ii] =acc.value_p[jj,ii] +1
        C_current<- C_prime
        p_current<- p_prime
        A_current <- A_prime
      }

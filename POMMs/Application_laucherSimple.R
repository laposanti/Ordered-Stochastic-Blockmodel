...
for(application in c('Tennis_data', "Citation_data")){
  ...
  if('Simple' %in% choose_model_to_estimate){
    ...
    for(i in 1:length(K_est)){
      saving_directories[[i]]<- paste0("./results/application/",application,"/Simple/K", K_est[[i]],"//")
      chains_seeds[[i]] = 20+i
    }
    ...
    cat(paste0("Estimation of the Simple model, K=", K_est, "\n"))
    ...
    chains <- adaptive_MCMC_orderstats_powerposterior(Y_ij = Y_ij, N_ij = N_ij,n_temperatures = n_temperatures,
                                                      ...
                                                      seed = chains_seeds, est_model = 'Simple', custom_init = custom_init)
